...
from tests.test_shopping_list_item.schemas_utils import (
    ...
    _assert_websocket_shopping_list_item_create_response_schema,
)
...
@pytest.mark.anyio
async def test_websocket_shopping_list_item_create_success(
    async_client,
    db_session,
    user_factory,
    family_factory,
    ...
    """Test WebSocket receives real-time creation events on shopping list items."""
    user = user_factory()
    family = family_factory()
    family_member_factory(family_id=family.id, user_id=user.id)
    shopping_list = shopping_list_factory(family_id=family.id)
    ...
    async with aconnect_ws(
        ...
        item_payload = shopping_list_item_create_payload_factory(
            shopping_list_id=str(shopping_list.id), name="Juice"
        )
        ...
        assert response["event_type"] == "user_created_shopping_list_item"
        assert response["data"]["id"] == str(shopping_list_item.id)
        assert response["data"]["name"] == item_payload["name"]

        _assert_websocket_shopping_list_item_create_response_schema(response)
