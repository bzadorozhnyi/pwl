...
async def test_retrieve_all_shopping_list_items_authentication_required(
    ...
    response = await async_client.get(
        f"/api/shopping-list-items/?shopping_list_id={shopping_list.id}"
    )
...
async def test_list_shopping_list_items_nonexistent_shopping_list_returns_404(
    ...
    response = await async_client.get(
        f"/api/shopping-list-items/?shopping_list_id={uuid.uuid4()}",
...
async def test_cannot_list_items_from_other_family_shopping_list(
    ...
    response = await async_client.get(
        f"/api/shopping-list-items/?shopping_list_id={shopping_list_other_family.id}",
        headers={"authorization": f"Bearer {access_token}"},
    )
...
async def test_paginated_list_shopping_list_items_multiple_pages(
    ...
    response = await async_client.get(
        f"/api/shopping-list-items/?shopping_list_id={shopping_list.id}&page=1&page_size=10",
        headers=headers,
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
